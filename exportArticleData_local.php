...
function writeCategoryPaths($ordernumber, $shop, $path){
    $sql = "INSERT INTO `shop_categories`(`ordernumber`, `shop`, `categorypath`) 
            VALUES ('".$ordernumber."','".$shop."','".$path."')";
...
function getCategories($shop, $artikelNr){
    ...
    if($shop == 'getgoods'){
        $db_shop_db = SHOP_DB;
        $image_path = 'http://admin.devel-shop.getgoods.de/images/articles';
    } else if ($shop == 'hoh'){
        $db_shop_db = SHOP_DB_HOH;
        $image_path = 'http://admin.hoh.de/images/articles';
    } else {
        print '<br>Fehler: Shop unbekannt!';
        exit();
    }
